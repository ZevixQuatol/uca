...
`uca.client.mode` 支持两种模式：

- `partial`：默认值，只有使用 `@UCAResponse` 标记的接口接受 UCA 服务调用；
- `full`：所有现有 Controller 接口都接受经过签名验证的 UCA 服务调用，已有项目无需逐个修改接口。

`full` 只影响 UCA 服务调用。普通前端或外部请求仍保持原项目的访问控制、响应状态和响应体，不会因为接入 UCA 被统一包装。
...
- `10006`：目标连接失败；
- `10007`：目标超时；
- `10008`：`partial` 模式下目标接口没有 `@UCAResponse`；
- `10009`：缺少服务身份；
- `10010`：服务签名无效；
